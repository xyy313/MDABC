...
public:
	MOEAD();
	MOEAD(int _H, int _NeighborhoodSize, int _NeighborhoodTrial, int _AgeTrial, int _MaximumCPU);
	~MOEAD();
public:
	void Gen2WV_NBI(); //generate weight vectors for two objectives
	void UpdateParetoPouplation(Individual& Ind);//完工时间
	void UpdateParetoPopulation();//能耗
	void InitializePopulation();  //initialize the population
	void EvolvePopulation();      //Evolve the population
...
MOEAD::MOEAD(int _H, int _NeighborhoodSize, int _NeighborhoodTrial, int _AgeTrial, int _MaximumCPU)
{
	this->H = _H;
...
void MOEAD::TwoPointsXoverAndMutation(Individual& Ind1, Individual& Ind2, Individual& ResultInd)
{
...
void MOEAD::UpdateParetoPouplation(Individual& Ind)
{
...
void MOEAD::UpdateParetoPopulation()
{
...
void MOEAD::NondominatedSorting()
{
	for (int i = 0; i < PopulationSize; i++)
	{
		Population[i].n = 0;
...
void MOEAD::OutputParetoPopulation(int ins, double costTime)
{
	...
	char outfile[40];
	sprintf_s(outfile, "results.moead\\%d_%d_%d_%d.txt", pJob, pStage, pType, ins);
